...
class AdminController extends BaseAdminController {
    ...
    public function indexAction(Request $request) {
        ...
        $config = $this->getParameter($this->entity['name']);
        $action = $request->query->get('action', 'list');
        if (!$this->isActionAllowed($action)) {
            throw new ForbiddenActionException(array('action' => $action, 'entity' => $this->entity['name']));
        }
        $em = $this->getDoctrine()->getManager();
        $count = $em->getRepository($config['entity_name'])->countByAll($config, $_GET);
        ...
        if ($action == 'list') {
            return $this->render('pxBackendBundle:Default:list.html.twig', array(
                        'showTable' => ($count > 0 ? true : false),
                        'config' => $config,
            ));
        }
    ...
    /**
     * @Route("/ajax/load", name="admin_table_list")
     * @Method("GET")
     */
    public function listTableAction(Request $request) {
        ...
        $config = $this->getParameter($this->entity['name']);
        $_service = $this->get('listing_service');
        $delete_form_template = $this->createDeleteForm($this->entity['name'], '__id__')->createView();
        $fields = $this->entity['list']['fields'];
        $result = $_service->getLines($config, $_GET, $fields, $delete_form_template);
    ...
    protected function initialize(Request $request) {
        ...
        $this->entity['templates']['new'] = "pxBackendBundle:Default:new.html.twig";
        $this->entity['templates']['edit'] = "pxBackendBundle:Default:edit.html.twig";
        $this->entity['templates']['list'] = "pxBackendBundle:Default:list.html.twig";
    }
    ...
    protected function editAction() {
        ...
        if ($editForm->isValid()) {
            ...
            $this->get('session')->getFlashBag()->add('success', $this->get('translator')->trans('entity.updated'));
    ...
    protected function newAction() {
        ...
        if ($newForm->isValid()) {
            ...
            $refererUrl = $this->request->query->get('referer', '');
            $this->get('session')->getFlashBag()->add('success', $this->get('translator')->trans('entity.created'));
            return !empty($refererUrl) ? $this->redirect(urldecode($refererUrl)) : $this->redirect($this->generateUrl('easyadmin', array('action' => 'list', 'entity' => $this->entity['name'])));
        }
    ...
    protected function deleteAction() {
        ...
        if ($form->isValid()) {
            ...
            $this->em->flush();
            $this->get('session')->getFlashBag()->add('success', $this->get('translator')->trans('entity.deleted'));
    ...
    protected function createEntityForm($entity, array $entityProperties, $view) {
        if (method_exists($this, $customMethodName = 'create' . $this->entity['name'] . 'EntityForm')) {
            ...
            if (!$form instanceof FormInterface) {
                throw new \UnexpectedValueException(sprintf(
                        'The "%s" method must return a FormInterface, "%s" given.', $customMethodName, is_object($form) ? get_class($form) : gettype($form)
                ));
            }
        ...
        if (!$formBuilder instanceof FormBuilderInterface) {
            throw new \UnexpectedValueException(sprintf(
                    'The "%s" method must return a FormBuilderInterface, "%s" given.', 'createEntityForm', is_object($formBuilder) ? get_class($formBuilder) : gettype($formBuilder)
            ));
        }
    ...
    private function redirectToBackendHomepage() {
        ...
        $url = isset($homepageConfig['url']) ? $homepageConfig['url'] : $this->get('router')->generate($homepageConfig['route'], $homepageConfig['params']);
